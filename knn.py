...
with confidence the presence of cancer 
...
Success Criteria: 
Business success criteria: Increase the correct diagnosis of cancer at 
least by 96% of patients
Machine Learning success criteria: Achieve an accuracy of atleast 98%
Economic success criteria: Reducing medical expenses will improve trust of 
patients and thereby hospital will see an increase in revenue by atleast 12%
...
X_train_ary = np.array(X_train)
X_test_ary = np.array(X_test)
...
pred_train = knn.predict(X_train_ary)  # Predict on train data

pred_series = pd.Series(pred_train)
...
for i in range(1, 50, 2):
    ...
    train_acc = np.mean(neigh.predict(X_train_ary) == Y_train)
    test_acc = np.mean(neigh.predict(X_test_ary) == Y_test)
...
KNN_new = grid.fit(X_train_ary, Y_train) 
...
# Predict the class on test data
pred = KNN_new.predict(X_test_ary)
pred
